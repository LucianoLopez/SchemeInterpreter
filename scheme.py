...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # BEGIN PROBLEM 5
        "*** REPLACE THIS LINE ***"
        function = scheme_eval(first, env)
        arguments = rest.map(lambda operand: scheme_eval(operand, env))
        check_procedure(function)
        return scheme_apply(function, arguments, env)
...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) > len(vals):
            raise SchemeError('insufficient number of values')
        elif len(vals) > len(formals):
            raise SchemeError('too many values')
        while formals:
...
def make_let_frame(bindings, env):
    ...
    while bindings:
        bind = bindings.first
        check_form(bind, 1, 2)
        check_formals(bind.first)
        if not scheme_symbolp(bind.first):
            raise SchemeError
        param = Pair(bind.first, param)
        val = Pair(scheme_eval(bind.second.first, env), val)
        bindings = bindings.second
    return env.make_child_frame(param, val)
    # END PROBLEM 15
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    while isinstance(result, Thunk):
        ...
        if (scheme_symbolp(first) and first in SPECIAL_FORMS):
            ...
            result = scheme_apply(function, args, env)
            # END Extra Credit
    return result
...
def scheme_load(*args):
    ...
    read_eval_print_loop(next_line, m, quiet=quiet)
