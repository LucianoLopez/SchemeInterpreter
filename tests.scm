...
(gcd 69 84)
; expect 3
(define error-lambda (lambda (9) (9)))
; expect Error: Non-symbol: 9
'(1 (3 five . (7 . 9)))
; expect (1 (3 five 7 . 9))
(let ((a 4) (b 5))
  (let ((c (+ a b)))
...
; expect 9 
(let ((n 58)) x 2 9)
; expect Error: unknown identifier: x
(let ((n 58)) n 2 9)
; expect 9
...
(factorial 6)
; expect 720
((lambda (b c) (* (+ 2 b) c)) 4 8)
; expect 48
(car (cdr-stream(cons-stream 1 (cons-stream 2 nil))))
; expect 2
(cons-stream 1 2)
...
(car (cdr (quote (a b c))))
; expect b
(car (cdr (cdr (quote (a b c)))))
; expect c
(car (cons 1 nil))
; expect 1
(car (cons 0 (cons 4 (cons 5 nil))))
; expect 0
(print 'doesthiswork)
; expect doesthiswork
(begin (+ 2 3) (+ 5 6))
...
(or 5 2 1)
; expect 5
(or False (- 1 1) 1)  ; 0 is a true value in Scheme
; expect 0
(or 4 True (/ 1 0))  ; short-circuiting behavior of or
; expect 4
(or)
; expect False
...
(g 3 7)
; expect 13
(define f (mu (x) (+ (+ y y) (- x y))))
; expect f
(define g (mu (y) (+ (* x 2) y)))
; expect g
(define h (lambda (x y) (* (f (+ x y)) (g (- x y)))))
; expect h
(h 2 4)
; expect 20 
